...
1. то, что пользователь дал в сообщении;
2. любой CSV в `leads/` (в sample-case это `leads/funnel-12mo.csv`);
3. если и там пусто — `skills/audit-funnel/demo-funnel.csv`, и пометь все выводы как учебные.
...
1. **Прочитай `signals/**/` (все три папки) и `CLAUDE.md`.** Критерии берутся оттуда. Если файлов нет или аудит
...
6. **Допиши в `signals/<маршрут>/`** только то, что БОЛЬШЕ одного аккаунта: «лиды с формы "запросить демо"
